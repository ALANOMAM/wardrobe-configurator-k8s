...
function RightSidePanel({ onClick, positionProp, dimensionsPropInMeters }) {
  const gltf = useGLTF("/wardrobe-img/rightPanel.gltf");
  ...
  useEffect(() => {
    ...
    console.log(
      " Right side panel dimensions in meters (W x H x D):",
      scaleX,
      scaleY,
      scaleZ
    );
  ...
  return (
    <group position={panelInfo.position} rotation={[0, -Math.PI / 2, 0]}>
      <primitive
        ref={meshRef}
        object={gltf.scene}
