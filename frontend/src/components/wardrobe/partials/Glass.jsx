...
function Glass({ onClick, positionProp, dimensionsPropInMeters }) {
  const gltf = useGLTF("/wardrobe-img/glass.gltf");
  ...
  useEffect(() => {
    ...
    console.log(
      "Glass dimensions in meters (W x H x D):",
      scaleX,
      scaleY,
      scaleZ
    );
  ...
  return (
    <group position={panelInfo.position} rotation={[0, Math.PI / 3, 0]}>
      <primitive
        ref={meshRef}
        object={gltf.scene}
